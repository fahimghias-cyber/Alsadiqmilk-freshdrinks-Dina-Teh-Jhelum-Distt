...
function openConfirmationModal(order) {
  ...
  const whatsappMsg = 
    ...
    `📝 *Special Note:* ${encodeURIComponent(order.customer.notes || 'None')}%0A%0A` +
    `🛒 *ITEMS ORDERED:*%0A${itemsSummaryText}%0A%0A` +
    `💰 *TOTAL PAYABLE (Counter / COD):* *Rs. ${order.totalAmount}*%0A` +
    `----------------------------------------%0A` +
    `✅ _Please confirm my booking. Thank you!_`;

  const num1Clean = formatWaPhone(store.whatsappSettings.num1 || "03709589018");
  const num2Clean = formatWaPhone(store.whatsappSettings.num2 || "03421008375");

  const url1 = `https://wa.me/${num1Clean}?text=${whatsappMsg}`;
  const url2 = `https://wa.me/${num2Clean}?text=${whatsappMsg}`;
  ...
  if (btnWa1) {
    btnWa1.href = url1;
    btnWa1.innerHTML = `<i class="fa-brands fa-whatsapp"></i> Send to WhatsApp 1 (${store.whatsappSettings.num1 || '0370-9589018'})`;
  }

  if (btnWa2) {
    btnWa2.href = url2;
    btnWa2.innerHTML = `<i class="fa-brands fa-whatsapp"></i> Send to WhatsApp 2 (${store.whatsappSettings.num2 || '0342-1008375'})`;
  }
      ...
      <div class="order-staff-card status-${order.status}" id="order-card-${order.id}">
        ...
        <div class="cust-info-block">
          ...
          <div class="cust-contact-line">
            ...
            <a href="https://wa.me/${formatWaPhone(order.customer.phone)}" target="_blank" style="color:#25d366;"><i class="fa-brands fa-whatsapp"></i> WhatsApp</a>
          </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("saveWhatsAppSettingsBtn").addEventListener("click", () => {
    ...
    showToast("WhatsApp dispatch numbers updated successfully!", "success");
    whatsappSettingsModal.classList.remove("active");
  });
